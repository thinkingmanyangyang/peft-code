...
class LoraConfig(PeftConfig):
    ...
    group_size: int = field(default=8, metadata={"help": "Group size of GLora"})
    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            ...
            # group lora param
            "use_glora": lora_config.use_glora,
...
class LoraLayer:
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights,
        ...
        self.use_glora[adapter_name] = use_glora
        ...
        if r > 0:
            if use_glora:
                ...
                weight_A = torch.randn((group_size, group_in_features, r), device=self.weight.device)
                weight_B = torch.randn((group_size, r, group_out_features), device=self.weight.device)
                ...
                rgg = torch.randn((r, group_size, group_size),
                                  device=self.weight.device)
                grr = torch.randn((group_size, r, r),
                                  device=self.weight.device)
    ...
    def reset_glora_parameters(self, adapter_name):
        ...
        rank, group_size = self.r[adapter_name], self.group_size[adapter_name]
        ...
        for i in range(group_size):
            ...
            # nn.init.kaiming_uniform_(self.lora_grr[adapter_name][i])
        
        # for i in range(rank):
        #     nn.init.kaiming_uniform_(self.lora_rgg[adapter_name][i])
            # nn.init.orthogonal_(self.rgg[adapter_name][i])
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
            if self.use_glora[self.active_adapter]:
                ...
                result += group_b * self.scaling[self.active_adapter]
