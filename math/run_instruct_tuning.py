...
import datasets
import numpy as np
from peft_lora import LoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    adapter_name_or_path: Optional[str] = field(default=None, metadata={"help": (
    ...
    merge: Optional[bool] = field(default=False, metadata={
        "help": "Merge the PiSSA adapter to the residual model or LoRA to the base model"}, )
...
def build_model(script_args, checkpoint_dir):
    ...
    if script_args.use_lora:
        if checkpoint_dir is not None:
            ...
            lora_config = LoraConfig.from_pretrained(script_args.model_name_or_path,
